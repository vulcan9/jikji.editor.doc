# Table of contents

* [Jik-ji API for Javacript](README.md)

## Tutorial

* [API 사용 방법](tutorial/tutorial.md)
* [이벤트 설정창에서 코드 작성](tutorial/code-in-editor.md)
* [외부 JS 파일에 코드 작성](tutorial/code-in-js.md)
* [이벤트 설정창에서 코드 작성 도움말](tutorial/eventwindow/README.md)
  * [동기/비동기 실행](tutorial/eventwindow/sync.md)
  * [미리 정의된 변수](tutorial/eventwindow/properties.md)
* [캡슐화 기능 사용하기](tutorial/capsulize/README.md)
  * [컴포넌트 만들기](tutorial/capsulize/create.md)
  * [컴포넌트 Export/Import](tutorial/capsulize/export-import.md)
  * [하위 요소에 값 전달](tutorial/capsulize/value-chain.md)
* [Drag & Drop 기능 사용하기](tutorial/drag-and-drop.md)
* [콤포넌트 경로 참조 변수](tutorial/export-root-path.md)
* [Element에서 JS 파일 동적으로 로드](tutorial/element-js.md)

## Sample

* [토글 버튼](sample/togglebutton.md)
* [마우스 드래그 기능](sample/mousedrag.md)
* [youtube API 활용하기](sample/youtube-api.md)

## API Reference

* [공통 API](api-reference/jjapi.md)
* [Animation 객체](api-reference/animationapi.md)
* [Window API ($this)](api-reference/window-api-usdthis/README.md)
  * [book 객체](api-reference/window-api-usdthis/book.md)
  * [media 객체](api-reference/window-api-usdthis/media.md)
  * [animation 객체](api-reference/window-api-usdthis/animation.md)
* [Document API](api-reference/document-api.md)
* [Element 공통 API](api-reference/elementapi/README.md)
  * [animation 객체](api-reference/elementapi/animation.md)
  * [capsule 객체](api-reference/elementapi/capsule.md)
  * [dnd 객체 (Drag \&Drop )](api-reference/elementapi/dnd-drag-and-drop.md)
* [Text Element API](api-reference/text-element-api.md)
* [Input Element API](api-reference/input-element-api.md)
* [Image Element API](api-reference/image-element-api.md)
* [Web Element API](api-reference/web-element-api.md)
* [Video/Audio Element API](api-reference/mediaapi.md)
